...
module.exports = {
  async up(queryInterface, Sequelize) {
    try {
      // 1. Convert all enum columns to TEXT first
      ...
      // 2. Drop ALL existing duplicate enums
      await queryInterface.sequelize.query(`
      ...
      // 3. Create single correct enum for each type
      await queryInterface.sequelize.query(`
      ...
      // 4. Convert columns back to new enums with proper mapping
      await queryInterface.sequelize.query(`
        ALTER TABLE "Leads"
  ...
  async down(queryInterface, Sequelize) {
    // Convert both back to text in case we need to rollback
    await queryInterface.sequelize.query(`
      ALTER TABLE "Leads"
      ALTER COLUMN status TYPE TEXT
      USING status::TEXT;
