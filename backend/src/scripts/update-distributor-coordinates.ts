import sequelize from '../config/sequelize';
import Distributor from '../models/Distributor';
import * as dotenv from 'dotenv';

dotenv.config();

// Coordinates data from the frontend cache
const coordinatesData: { [key: string]: { lat: number; lng: number } } = {
  'Chandigarh-Chandigarh': { lat: 30.7334421, lng: 76.7797143 },
  'Delhi-Delhi NCR': { lat: 28.6139, lng: 77.209 },
  'Surat-Gujarat': { lat: 21.2094892, lng: 72.8317058 },
  'Rajkot-Gujarat': { lat: 22.3053263, lng: 70.8028377 },
  'Ahemdabad-Gujarat': { lat: 23.0225, lng: 72.5714 },
  'Vadodara-Gujarat': { lat: 22.2973142, lng: 73.1942567 },
  'Kaithal-Haryana': { lat: 29.7856, lng: 76.3984 },
  'Gurugram-Haryana': { lat: 28.4595, lng: 77.0266 },
  'Hisar-Haryana': { lat: 29.1492, lng: 75.7217 },
  'Ranchi-Jharkhand': { lat: 23.3441, lng: 85.3096 },
  'Dhanbad-Jharkhand': { lat: 23.7957, lng: 86.4304 },
  'Bengaluru-Karnataka': { lat: 12.9716, lng: 77.5946 },
  'Belgaum-Karnataka': { lat: 15.8497, lng: 74.4977 },
  'Gwalior-Madhya Pradesh': { lat: 26.2183, lng: 78.1828 },
  'Indore-Madhya Pradesh': { lat: 22.7196, lng: 75.8577 },
  'Bhopal-Madhya Pradesh': { lat: 23.2599, lng: 77.4126 },
  'Jabalpur-Madhya Pradesh': { lat: 23.1815, lng: 79.9864 },
  'Mumbai-Maharashtra': { lat: 19.0760, lng: 72.8777 },
  'Pune-Maharashtra': { lat: 18.5204, lng: 73.8567 },
  'Kolhapur-Maharashtra': { lat: 16.7050, lng: 74.2433 },
  'Nashik-Maharashtra': { lat: 19.9975, lng: 73.7898 },
  'Thane-Maharashtra': { lat: 19.2183, lng: 72.9781 },
  'Ludhiana-Punjab': { lat: 30.9010, lng: 75.8573 },
  'Bathinda-Punjab': { lat: 30.2110, lng: 74.9455 },
  'Jalandhar-Punjab': { lat: 31.3260, lng: 75.5762 },
  'Amritsar-Punjab': { lat: 31.6340, lng: 74.8723 },
  'Udaipur-Rajasthan': { lat: 24.5854, lng: 73.7125 },
  'Jaipur-Rajasthan': { lat: 26.9124, lng: 75.7873 },
  'Hyderabad-Telangana': { lat: 17.3850, lng: 78.4867 },
  'Kanpur-Uttar Pradesh': { lat: 26.4499, lng: 80.3319 },
  'Jhansi-Uttar Pradesh': { lat: 25.4484, lng: 78.5685 },
  'Agra-Uttar Pradesh': { lat: 27.1767, lng: 78.0081 },
  'Rampur-Uttar Pradesh': { lat: 28.7984, lng: 79.0270 },
  'Bareilly-Uttar Pradesh': { lat: 28.3670, lng: 79.4304 },
  'Kashipur-Uttarakhand': { lat: 29.2104, lng: 78.9619 },
  'Kolkata-West Bengal': { lat: 22.5726, lng: 88.3639 },
  'Siliguri-West Bengal': { lat: 26.7271, lng: 88.3953 },
  'Bengluru-Karnataka': { lat: 12.9716, lng: 77.5946 },
  'Singrauli-Madhya Pradesh': { lat: 24.2431343, lng: 82.3827672091869 },
  'Vasai-Maharashtra': { lat: 19.3428238, lng: 72.805441 },
  'Latur-Maharashtra': { lat: 18.355334749999997, lng: 76.75489974062862 },
  'Gondia-Maharashtra': { lat: 21.455228, lng: 80.1962729 },
  'Nagpur-Maharashtra': { lat: 21.1498134, lng: 79.0820556 },
  'Lucknow-Uttar Pradesh': { lat: 26.8381, lng: 80.9346001 },
  'Meerut-Uttar Pradesh': { lat: 29.0018557, lng: 77.76796714992041 },
  'Dehradun-Uttarakhand': { lat: 30.3255646, lng: 78.0436813 },
  'Haldwani-Uttarakhand': { lat: 29.2144809, lng: 79.5279012 },
  'Srinagar-Kashmir': { lat: 34.0747444, lng: 74.8204443 },
  'Satna-Madhya Pradesh': { lat: 24.6005, lng: 80.8322 },
  'Patiala-Punjab': { lat: 30.3398, lng: 76.3869 },
  'Dhule-Maharashtra': { lat: 20.9042, lng: 74.7775 }
};

async function updateDistributorCoordinates() {
  try {
    await sequelize.authenticate();
    console.log('✓ Connected to database\n');

    // Get all distributors
    const distributors = await Distributor.findAll();
    console.log(`Found ${distributors.length} distributors\n`);

    let updatedCount = 0;
    let skippedCount = 0;
    let notFoundCount = 0;

    for (const distributor of distributors) {
      const key = `${distributor.city}-${distributor.state}`;
      const coordinates = coordinatesData[key];

      if (coordinates) {
        // Check if distributor already has coordinates
        if (distributor.latitude && distributor.longitude) {
          console.log(`✓ ${distributor.name} (${key}) already has coordinates`);
          skippedCount++;
        } else {
          // Update the distributor with coordinates
          await distributor.update({
            latitude: coordinates.lat,
            longitude: coordinates.lng
          });
          console.log(`✅ Updated ${distributor.name} (${key}) with lat: ${coordinates.lat}, lng: ${coordinates.lng}`);
          updatedCount++;
        }
      } else {
        console.log(`❌ No coordinates found for ${distributor.name} (${key})`);
        notFoundCount++;
      }
    }

    console.log('\n--- Summary ---');
    console.log(`✅ Updated: ${updatedCount} distributors`);
    console.log(`⏭️  Skipped: ${skippedCount} distributors (already had coordinates)`);
    console.log(`❌ Not found: ${notFoundCount} distributors (no coordinates in cache)`);

    // List distributors without coordinates
    if (notFoundCount > 0) {
      console.log('\nDistributors without coordinates:');
      for (const distributor of distributors) {
        const key = `${distributor.city}-${distributor.state}`;
        if (!coordinatesData[key] && !distributor.latitude) {
          console.log(`- ${distributor.name} (${key})`);
        }
      }
    }

  } catch (error) {
    console.error('Error updating coordinates:', error);
  } finally {
    await sequelize.close();
  }
}

updateDistributorCoordinates();